...
public class GameServiceImpl implements GameService
{
    ...
    public Game create(Game game) throws DAOException, SQLException
    {
        ...
        try {
            ...
            Game g1 = dao.create(connection, game);
            connection.commit();
            return g1;
        } catch (Exception ex) {
            connection.rollback();
    ...
    public Game retrieve(long gameID) throws DAOException, SQLException
    {
        ...
        try {
            connection.setAutoCommit(false);
            Game g1 = dao.retrieve(connection, gameID);
            connection.commit();
            return g1;
        } catch (Exception ex) {
            connection.rollback();
    ...
    public int update(Game game) throws DAOException, SQLException
    {
        ...
        try {
            connection.setAutoCommit(false);
            int count = dao.update(connection, game);
            connection.commit();
            return count;
        } catch (Exception ex) {
            connection.rollback();
    ...
    public int delete(long gameID) throws DAOException, SQLException
    {
        ...
        try {
            connection.setAutoCommit(false);
            int count = dao.delete(connection, gameID);
            connection.commit();
            return count;
        } catch (Exception ex) {
            connection.rollback();
    ...
    public List<Game> retrieveByTitle(String titlePattern) throws DAOException, SQLException
    {
        ...
        try {
            connection.setAutoCommit(false);
            List<Game> g1 = dao.retrieveByTitle(connection, titlePattern);
            connection.commit();
            return g1;
        } catch (Exception ex) {
            connection.rollback();
    ...
    public List<Game> retrieveByReleaseDate(Date start, Date end) throws DAOException, SQLException
    {
        ...
        try {
            connection.setAutoCommit(false);
            List<Game> g1 = dao.retrieveByReleaseDate(connection, start, end);
            connection.commit();
            return g1;
        } catch (Exception ex) {
            connection.rollback();
